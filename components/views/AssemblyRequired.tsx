  ...
  return (
    <div className="relative z-10 flex flex-col gap-6 max-w-5xl mx-auto w-full">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-2xl font-bold text-foreground">Required Components</h2>
          ...
          <Card className="border-border bg-card overflow-hidden">
            ...
            <div className="p-4 flex flex-col gap-3">
              {order.map((item, index) => (
                <div key={item.id} className="flex gap-2 items-center bg-background p-2 rounded-lg border border-border">
                  <select
                    className="flex-1 bg-transparent text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-indigo-500/50 rounded p-2"
                    value={item.productId}
                    onChange={e => updateLine(item.id, 'productId', e.target.value)}
                  >
                    <option value="" disabled className="bg-card">Select Product...</option>
                    {products.map(p => (
                  ...
                  <input
                    type="number"
                    min="1"
                    className="w-20 bg-transparent text-center text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-indigo-500/50 rounded p-2 border-l border-border"
                    value={item.qty}
                  ...
                  <button
                    ...
                    className="p-2 text-muted-foreground hover:text-red-600 dark:text-red-400 disabled:opacity-50 transition-colors"
                  >
                    <Trash2 size={16} />
                  </button>
                </div>
              ...
              <Button
                variant="outline"
                className="w-full mt-2 border-dashed border-border text-muted-foreground hover:text-foreground hover:border-indigo-500/50 hover:bg-indigo-500/10"
                onClick={addLine}
              >
            ...
            <div className="flex-1 p-4 overflow-y-auto min-h-[300px]">
              {aggregatedBOM.length === 0 ? (
                ...
                <div className="flex flex-col gap-2">
                  <div className="grid grid-cols-12 gap-2 px-3 py-2 text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">
                  ...
                  {aggregatedBOM.map(req => {
                    ...
                    return (
                      <div key={req.name} className="grid grid-cols-12 gap-2 items-center bg-background px-3 py-3 rounded-lg border border-border group hover:border-border transition-colors">
                        <div className="col-span-6">
                          <p className="text-sm font-semibold text-foreground">{req.name}</p>
                          <p className="text-xs text-muted-foreground mt-0.5">Stock: {req.stock} {req.unit}</p>
                        </div>
                        <div className="col-span-3 text-right">
                          <span className="text-sm font-bold text-indigo-400">{req.required}</span>
                          <span className="text-xs text-muted-foreground ml-1">{req.unit}</span>
                        </div>
                        <div className="col-span-3 text-right">
                          {shortfall > 0 ? (
                            <span className="text-sm font-bold text-red-600 dark:text-red-400 bg-red-100 dark:bg-red-500/10 px-2 py-1 rounded">
                              -{shortfall}
                            </span>
                          ) : (
