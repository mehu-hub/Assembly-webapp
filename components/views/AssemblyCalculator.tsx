  ...
  return (
    <div className="relative z-10 flex flex-col gap-6 max-w-4xl mx-auto w-full">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-2xl font-bold text-foreground">Assembly Cost Calculator</h2>
      ...
      <Card className="border-border bg-card p-6">
        <div className="grid grid-cols-1 sm:grid-cols-12 gap-4 items-end">
          <div className="sm:col-span-6 flex flex-col gap-2">
            <label className="text-xs font-semibold text-muted-foreground uppercase">Product to Calculate</label>
            <select
              className="h-10 px-3 bg-background text-foreground border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500/50"
          ...
          <div className="sm:col-span-3 flex flex-col gap-2">
            <label className="text-xs font-semibold text-muted-foreground uppercase">Run Size</label>
            <input
              type="number"
              min="1"
        ...
        <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <Card className="border-border bg-card p-5">
              <p className="text-xs font-bold text-muted-foreground uppercase mb-2">Total BOM Cost</p>
            ...
            <Card className="border-border bg-card p-5">
              <p className="text-xs font-bold text-muted-foreground uppercase mb-2">Est. Revenue</p>
              <div className="flex items-center gap-2">
            ...
            <Card className={`border-border bg-card p-5 ${calculationResult.profit > 0 ? 'border-emerald-500/20' : 'border-red-500/20'}`}>
              <p className="text-xs font-bold text-muted-foreground uppercase mb-2">Gross Profit</p>
              <div className="flex items-center gap-2">
          ...
          <Card className="border-border bg-card overflow-hidden">
            <div className={`p-4 border-b border-border flex items-center justify-between ${calculationResult.isPossible ? 'bg-emerald-500/5' : 'bg-red-500/5'}`}>
              <div className="flex items-center gap-2">
                ...
                <tbody className="divide-y divide-white/5">
                  {calculationResult.details.map((row: any, i: number) => (
                    <tr key={i} className="hover:bg-white/2 transition-colors">
                      ...
                      <td className="px-4 py-3 text-right text-muted-foreground">{row.stock}</td>
                      <td className="px-4 py-3 text-right">
                        {row.missing > 0 ? (
                          <span className="px-2 py-0.5 rounded bg-red-200 dark:bg-red-500/20 text-red-600 dark:text-red-400 font-bold">
                            {row.missing}
                          </span>
                        ) : '-'}
                      </td>
                      <td className="px-4 py-3 text-right text-muted-foreground">{row.price.toFixed(2)}</td>
                      <td className="px-4 py-3 text-right font-semibold text-foreground">{row.cost.toFixed(2)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
