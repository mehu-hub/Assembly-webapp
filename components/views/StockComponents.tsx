  ...
  return (
    <div className="relative z-10 flex flex-col gap-6 max-w-5xl mx-auto w-full">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-2xl font-bold text-foreground">Component Stock</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {isLoading ? (
          ...
          data.map(item => (
            <Card key={item.id} className="border-border bg-card hover:border-indigo-300 dark:border-indigo-500/20 transition-all duration-300">
              <CardHeader className="pb-3 border-b border-border flex flex-row items-center justify-between">
                <CardTitle className="text-sm font-bold text-foreground truncate pr-2">{item.name}</CardTitle>
                {item.totalQty === 0 ? (
              ...
              <CardContent className="pt-4 flex flex-col gap-2">
                <div className="flex justify-between items-center">
                  <span className="text-xs text-muted-foreground uppercase font-semibold">Total Stock</span>
                  <div className="flex items-baseline gap-1">
                    <span className={`text-2xl font-extrabold ${item.totalQty === 0 ? 'text-red-400' : 'text-foreground'}`}>
                      {item.totalQty}
                    </span>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className={`text-xs font-bold px-2 py-1 rounded ${
                    item.totalQty === 0 ? 'bg-red-100 dark:bg-red-500/10 text-red-400' : 
                    item.totalQty < 10 ? 'bg-amber-100 dark:bg-amber-500/10 text-amber-400' : 
                    'bg-emerald-100 dark:bg-emerald-500/10 text-emerald-400'
                  }`}>
