  ...
  return (
    <div className="relative z-10 flex flex-col gap-6 max-w-5xl mx-auto w-full">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-2xl font-bold text-white">Component Prices</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-2">
        <Card className="md:col-span-1 border-white/6 bg-[#0f1117] p-5">
          <p className="text-xs font-bold text-slate-500 uppercase mb-2">Total Inventory Value</p>
              ...
              <tbody className="divide-y divide-white/5">
                {data.map((item, i) => (
                  <tr key={i} className="hover:bg-white/2 transition-colors">
                    ...
                    <td className="px-4 py-3 text-right text-slate-400">{item.totalQty} {item.unit}</td>
                    <td className="px-4 py-3 text-right text-indigo-400 font-medium">{item.unitPrice.toFixed(2)}</td>
